    ...
    <TouchableOpacity style={styles.card} activeOpacity={0.9}>
      <View style={styles.imageContainer}>
        ...
        <BlurView intensity={20} tint="dark" style={styles.ratingBadge}>
          <Text style={styles.ratingText}>⭐ {item.score || item.rating || 'N/A'}</Text>
        </BlurView>
      </View>
      <View style={styles.cardContent}>
        <Text style={styles.cardTitle} numberOfLines={2}>{item.title || item.title_english}</Text>
        <View style={styles.statsRow}>
          <Text style={styles.statLabel}>🎬 {item.episodes || '?'} eps</Text>
          {item.year && <Text style={styles.statLabel}>📅 {item.year}</Text>}
        </View>
  ...
  const renderEmptyState = () => {
    ...
    return (
      <View style={styles.emptyContainer}>
        ...
        <Text style={styles.emptyTitle}>No seasonal anime found</Text>
